...
## Synonym/Antonym activity - matching game
## Match words with their synonyms/antonyms

# Matching pairs data
var matching_pairs = [
	{"word": "gigantic", "match": "enormous", "emoji": "🐘"},
	{"word": "happy", "match": "joyful", "emoji": "😊"},
	{"word": "quick", "match": "fast", "emoji": "⚡"}
]

var word_index: int = 4
var total_words: int = 5

# Game state
var selected_card = null  # {index: int, column: String, button: Button}
var matched_pairs = []  # Array of matched indices
...
func _setup_ui() -> void:
	...
	instruction.text = "Match the synonyms:"
...
func _try_match(card1: Dictionary, card2: Dictionary) -> void:
	...
	if is_match:
		...
		if matched_pairs.size() == matching_pairs.size():
			_show_completion()
	else:
...
func _on_next_pressed() -> void:
	print("Continue to antonyms")
	answer_submitted.emit()
	get_tree().change_scene_to_file("res://scenes/activities/Antonym.tscn")
