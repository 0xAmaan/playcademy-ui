...
## Spelling activity - multiple choice spelling options
...
func _setup_ui() -> void:
	...
	instruction.text = "Spell this word:"
	...
	# Create shuffled spelling options
	...
	for i in range(options.size()):
		...
		choice_btn.custom_minimum_size = Vector2(550, 70)
		...
		AssetLoader.apply_header_font(choice_btn, 22)
...
func _create_shuffled_options() -> Array:
	var options = [current_word["word"]]
	options.append_array(current_word["wrong_spellings"])

	# Shuffle the options
	options.shuffle()

	# Find the correct answer's new index
	correct_index = options.find(current_word["word"])
		...
		if button_container is CenterContainer:
			...
			if btn is Button:
				...
				if i == correct_index:
					...
					feedback_label.text = "Not quite. The correct spelling is: %s" % current_word["word"]
...
func _on_next_pressed() -> void:
	...
	spelling_submitted.emit()
	get_tree().change_scene_to_file("res://scenes/activities/FillBlank.tscn")
