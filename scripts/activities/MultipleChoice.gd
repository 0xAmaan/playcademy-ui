...
## Multiple Choice activity - pick the correct definition for a word
## Mock data for now (no backend)

# Mock word data
var current_word = {
	"word": "courageous",
	"correct_definition": "brave and willing to face danger or pain",
	"wrong_definitions": [
		"feeling or showing fear",
		"lacking in strength or ability",
...
func _setup_ui() -> void:
	...
	instruction.text = "What does this word mean?"
	...
	# Word card - match Flashcard styling
	var word_card = PanelContainer.new()
	word_card.custom_minimum_size = Vector2(600, 200)

	# Card styling with darker border (like Flashcard)
...
func _create_shuffled_options() -> Array:
	var options = [current_word["correct_definition"]]
	options.append_array(current_word["wrong_definitions"])

	# Shuffle the options
	options.shuffle()

	# Find the correct answer's new index
	correct_index = options.find(current_word["correct_definition"])
		...
		if button_container is CenterContainer:
			...
			if btn is Button:
				...
				if i == correct_index:
					...
					feedback_label.text = "Not quite. Try again next time!"
...
func _on_next_pressed() -> void:
	...
	get_tree().change_scene_to_file("res://scenes/activities/Spelling.tscn")
